...
class Game {
  constructor(grids) {
    ...
    this.switchGameHitbox = new Hitbox2D(70, 0, 150, canvas.height, "switch");
  ...
  updateCannon = () => {
    ...
    if (this.playerReady) {
      ...
      for (
        ...
        this.enemies.push(
          new Enemy(canvas.width - 128, canvas.height / 2, 128, 80, this)
        );
      }
    }
  ...
  playCollect() {
    ...
    if (this.useProcedualWorldCreation) {
      for (let i = 0; i < 20; i++) {
        console.log("Creating procedual world: " + (i / 20) * 100 + "%");
        let procedualTile = this.procedual.newTile(this.procedual.yPos);
        applyTextureData(procedualTile, this.grids[0].length);
      }
